...
try:
	shutil.rmtree('usb_modeswitch.d_bonovo')
except:
	pass
os.mkdir('usb_modeswitch.d_bonovo')
files = glob.glob('usb_modeswitch.d/*')
...
for f in files:
	fo = f.replace('.d/','.d_bonovo/').replace(':','_').replace('=','.').replace('#','')
	shutil.copy(f,fo)
	...
	fmk.write('LOCAL_SRC_FILES := usb_modeswitch.d_bonovo/%s\n'%name)
